/**
 * Given a set of wire segments, return a set of
 * points along w's path
 *
 * Input: R1,U1,L1,D1
 * Returns: [[1, 0], [1, 1], ...]
 * @param w
...
const trace = w => {
    ...
    let pos = [0, 0]
    let steps = 0
    w.trim().split(',').forEach(x => {
        let dir = x.charAt(0)
        let n = Number.parseInt(x.slice(1))
        for (i = 0; i < n; i++) {
            switch (dir) {
                case 'U':
                    pos[1] += 1
            ...
            let s = `${pos[0]}-${pos[1]}`
            if (visited[s] === undefined) {
                // Only update steps the first time we
                // visit a coord
                visited[s] = steps
            }
        }
...
const nearestCrossover = (w1, w2) => {
    let v1 = trace(w1)
    let v2 = trace(w2)

    Object.keys(v1).forEach(x => {
        if (Number.isInteger(v2[x])) {
            console.log(`pos: ${x}, steps w1: ${v1[x]}, steps w2: ${v2[x]}, total: ${v1[x] + v2[x]}`)
        }
    })
    // console.log(v1)
    // console.log(v2)
